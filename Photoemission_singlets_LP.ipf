...
SetVariable numpeaks, limits={1,15,1}, value= :Fitting:npeaks
Button PeakPar,  pos={40,84},size={120,20},title="enter peak parameters", proc=ParEnter

Button graph,  pos={40,120},size={120,20},title="Graph", proc=makegraph_singlets
Button fit,  pos={40,150},size={120,20},title="Fit", proc=makefit_singlets

end 

//Procedura che permette di inserire i parametri

Proc ParEnter(numpeaks)
String numpeaks
...
do
	...
	par_table[h*4-3][0]="Area "+num2str(h)+ " - K" + num2str(p)		//scrive nella colonna 0 della tabella i label dei parametri per i picchi Voigt
	par_table[h*4-2][0]="Gw "+num2str(h)+ " - K" + num2str(p)
	par_table[h*4-1][0]="Erg."+num2str(h)+ " - K" + num2str(p)
	par_table[h*4-0][0]="Lw "+num2str(h)+ " - K" + num2str(p) 
h+=1
while(1)
...
Proc makegraph_singlets(graph)
String graph
parameters_singlets()
selectXYwaves_singlets()
Variable/G Ydatalen
Ydatalen = numpnts($Ydata)
FuncFit/O/Q /L=(Ydatalen)  VoigtFitsinglet par $Ydata /X=$Xdata /D //grafica senza fare il fit
string fit_trace 
...
string voigtdd
	...
	do
		...
		vgt = w1[0] + w1[i*4-3]*(Voigt(w1[i*4-3+1]*($Xdata[p]-w1[i*4-3+2]),w1[i*4-3+3]))
...
Proc makefit_singlets(fit)
String fit 
selectXYwaves_singlets()
parameters_singlets()
hold_singlets()
constraints_singlets()
Limits_singlets() 
Variable/G Ydatalen=numpnts($Ydata)
String/G Fix = "\""+ParHold+"\""
FuncFit /H="Fix" /L=(Ydatalen) VoigtFitsinglet par $Ydata /X=$Xdata  /D  /C=Limits
par_table[][1]=num2str(par[p])
string fit_trace  
fit_trace = "fit_"+Ydata
ModifyGraph rgb($fit_trace)=(0,0,65535)
...
Function selectXYwaves_singlets()	
	String/G Ydata
