...
//w1[0]+w1[1]*(Voigt(w1[2]*(x-w1[3]),w1[4]) + (w1[5])*Voigt(w1[2]*(x-w1[3]+w1[6]),w1[4]))
...
//shape (but also affects the amplitude); w1[5] is the branching ratio; w1[6] is the spin orbit splitting in eV.
...
//w[5] is the branching ratio;
//w[6] is the spin orbit splitting 
//
//You can generate multiple peaks by appending 6 additional coefficient values to the coefficient wave for each peak.
// Each set of four is analogous to coefficients 1-6.

Function VoigtFitdoublet(w,x)
	Wave w; Variable x
	duplicate/O w, w1
	...
	do
		...
		r += w1[i]*(VoigtFunc(w1[i+1]*(x-w1[i+2]),w1[i+3])+ (w1[i+4])*VoigtFunc(w1[i+1]*(x-w1[i+2]+w1[i+5]),w1[i+3]))
		i+=6
	while(1)
	return r
End