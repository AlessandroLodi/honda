...
Proc MakeFitPanelSetup_DS_doublets()
...
Button PeakPar,  pos={40,84},size={120,20},title="enter peak parameters", proc=ParEnter_DS_doublets

Button graph,  pos={40,120},size={120,20},title="Graph", proc=makegraph_DS_doublets
Button fit,  pos={40,150},size={120,20},title="Fit", proc=makefit_DS_doublets

end 


Proc ParEnter_DS_doublets(numpeaks)
String numpeaks
...
do
	...
	par_table[h*6-5][0]="Amp "+num2str(h)+ " - K" + num2str(p)		//scrive nella colonna 0 della tabella i label dei parametri per i picchi
	par_table[h*6-4][0]="Asy "+num2str(h)+ " - K" + num2str(p)
	par_table[h*6-3][0]="Erg."+num2str(h)+ " - K" + num2str(p)
	par_table[h*6-2][0]="Gw "+num2str(h)+ " - K" + num2str(p) 
...
Function  parameters_DS_doublets()
...
Function constraints_DS_doublets()
NVAR gnpeaks = :Fitting:npeaks
Variable npeaks=gnpeaks
Wave/T par_table
...
Proc makegraph_DS_doublets(graph)
String graph
parameters_DS_doublets()
selectXYwaves_DS_doublets()
Variable/G Ydatalen
Ydatalen = numpnts($Ydata)
FuncFit/O/Q /L=(Ydatalen)  DoniachSunjicDoublet par $Ydata /X=$Xdata /D //grafica senza fare il fit
string fit_trace 
...
string dspdd
	...
	do
		...
		make/O/N=(nn) dspeakd
		dspdd=dspd+num2str(i)
		dspeakd = w1[i*6-5] * gamma(1-w1[i*6-5+1])/((w1[i*6-5+2]-$Xdata[p])*(w1[i*6-5+2]-$Xdata[p]) + w1[i*6-5+3]*w1[i*6-5+3]/4)^((1-w1[i*6-5+1])/2)*cos(pi*w1[i*6-5+1]/2+(1-w1[i*6-5+1])*atan((w1[i*6-5+2]-$Xdata[p])/w1[i*6-5+3])) 
		dspeakd = dspeakd + w1[i*6-5+4] *  w1[i*6-5] * gamma(1-w1[i*6-5+1])/((w1[i*6-5+2]-$Xdata[p]+w1[i*6-5+5])*(w1[i*6-5+2]-$Xdata[p]+w1[i*6-5+5]) + w1[i*6-5+3]*w1[i*6-5+3]/4)^((1-w1[i*6-5+1])/2)*cos(pi*w1[i*6-5+1]/2+(1-w1[i*6-5+1])*atan((w1[i*6-5+2]-$Xdata[p]+w1[i*6-5+5])/w1[i*6-5+3]))
		dspeakd = dspeakd + w1[0]
		duplicate/O dspeakd $dspdd
		CheckDisplayed $dspdd
		if ( V_flag != 1)							//checks if voigt doublet waves exist on the graph
			appendtograph $dspdd vs $Xdata         //if they not exist on the graph then they are plotted
		endif 
		i+=1
	while(1)
KillWaves dspeakd
...
Proc makefit_DS_doublets(fit)
String fit 
selectXYwaves_DS_doublets()
parameters_DS_doublets()
hold_DS_doublets()
constraints_DS_doublets()
Limits_DS_doublets() 
Variable/G Ydatalen=numpnts($Ydata)
String/G Fix = "\""+ParHold+"\""
FuncFit /H="Fix" /L=(Ydatalen) DoniachSunjicDoublet par $Ydata /X=$Xdata  /D  /C=Limits
par_table[][1]=num2str(par[p])
string fit_trace  
fit_trace = "fit_"+Ydata
ModifyGraph rgb($fit_trace)=(0,0,65535)
...
string dspdd
	...
	do
		...
		dspdd=dspd+num2str(i)
		dspeakd = w1[i*6-5] * gamma(1-w1[i*6-5+1])/((w1[i*6-5+2]-$Xdata[p])*(w1[i*6-5+2]-$Xdata[p]) + w1[i*6-5+3]*w1[i*6-5+3]/4)^((1-w1[i*6-5+1])/2)*cos(pi*w1[i*6-5+1]/2+(1-w1[i*6-5+1])*atan((w1[i*6-5+2]-$Xdata[p])/w1[i*6-5+3]))
		dspeakd += w1[i*6-5+4] *  w1[i*6-5] * gamma(1-w1[i*6-5+1])/((w1[i*6-5+2]-$Xdata[p]+w1[i*6-5+5])*(w1[i*6-5+2]-$Xdata[p]+w1[i*6-5+5]) + w1[i*6-5+3]*w1[i*6-5+3]/4)^((1-w1[i*6-5+1])/2)*cos(pi*w1[i*6-5+1]/2+(1-w1[i*6-5+1])*atan((w1[i*6-5+2]-$Xdata[p]+w1[i*6-5+5])/w1[i*6-5+3]))
		dspeakd += w1[0]
		duplicate/O dspeakd $dspdd
		CheckDisplayed $dspdd
		if ( V_flag != 1)							//checks if voigt doublet waves exist on the graph
...
Function selectXYwaves_DS_doublets()	
	String/G Ydata
